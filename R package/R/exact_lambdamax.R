# exact_lambdamax
exact_lambdamax = function(VMPLN_list,lambda_max_ref = NULL,
  ...
  n<-dim(VMPLN_list$obs_mat)[1]
  celltypes_num<-ncol(VMPLN_list$U_mat)
  p_GRN<-length(VMPLN_list$gene_GRN)
  p_nonGRN<-dim(VMPLN_list$obs_mat)[2] - length(VMPLN_list$gene_GRN)
  ...
  VMPLN_list1<-init_Theta(VMPLN_list,lambda_use = lambda,penalize_diagonal = FALSE,Theta_threshold = Theta_threshold, core_num = min(celltypes_num,core_num))
  
  ##
  VMPLN_list1 <- VMPLN_optim(celltypes_label_input = VMPLN_list1$celltypes_label,
                             U_mat_input = VMPLN_list1$U_mat,
                             mu_mat_input = VMPLN_list1$mu_mat,
                             m_mat_list_input = VMPLN_list1$m_mat_list,
                             s_sq_mat_list_input = VMPLN_list1$s_sq_mat_list,
                             pi_vec_input = VMPLN_list1$pi_vec,
                             obs_mat_input = VMPLN_list1$obs_mat,
                             S_depth_input = VMPLN_list1$S_depth,
                             gene_GRN_index_use_input = VMPLN_list1$gene_GRN_index_use,
                             Theta_mat_list_input = VMPLN_list1$Theta_mat_list,
                             zero_GRN_input = VMPLN_list1$zero_GRN,
                             if_zero = VMPLN_list1$zero_GRN_use,
                             log_det_vec_input = VMPLN_list1$log_det_vec,
                             lambda_use = lambda,
  ...
  Theta_nonzero_vec<-c()
  for(g in 1:ncol(VMPLN_list1$U_mat)){
    Theta_mat_sub<-as.matrix(VMPLN_list1$Theta_mat_list[[g]])[VMPLN_list1$gene_GRN_index_use,VMPLN_list1$gene_GRN_index_use]
  ...
  if(length(group_less)>0){
    ...
    while(con_0){
      ...
      ##
      VMPLN_list1<-init_Theta(VMPLN_list,lambda_use = lambda,penalize_diagonal = FALSE,Theta_threshold = Theta_threshold,
