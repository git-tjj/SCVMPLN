##VMPLN_init
VMPLN_init<-function(expression_profile,
                     celltypes_num,
                     celltypes_ref = NULL,
                     ls_est = "TSS",
                     gene_GRN,
  ...
  VMPLN_intseurat<-CreateSeuratObject(counts = expression_profile,
                                      project = "VMPLN_intseurat")
  VMPLN_intseurat<-NormalizeData(VMPLN_intseurat, verbose = verbose)
  
  VMPLN_intseurat<-FindVariableFeatures(VMPLN_intseurat, selection.method = "vst", 
                                        nfeatures = max(HVG_num,HVG_model_num), verbose = verbose)
  
  HVG_list<-VariableFeatures(VMPLN_intseurat)
  if(ls_est == "TSS"){
    S_depth<-as.vector(Matrix::colSums(expression_profile))/10000
  }
  if(ls_est == "GMPR"){
    S_depth<-as.vector(GMPR(as.matrix(Matrix::t(VMPLN_intseurat@assays$RNA@counts[which(rownames(VMPLN_intseurat@assays$RNA@counts) %in% HVG_list[1:HVG_num]),])))) 
  }
  if(ls_est == "none"){
    S_depth<-rep(1,length(S_depth))
  }
  
  VMPLN_intseurat <- ScaleData(VMPLN_intseurat, verbose = verbose)
  suppressWarnings(VMPLN_intseurat<- RunPCA(VMPLN_intseurat, npcs = npc, verbose = verbose))
  pca_embedding<-NULL
  if(cluster_method == "Kmeans"){
    pca_embedding<-matrix(as.vector(VMPLN_intseurat@reductions$pca@cell.embeddings),nrow = dim(VMPLN_intseurat@reductions$pca@cell.embeddings)[1],ncol = dim(VMPLN_intseurat@reductions$pca@cell.embeddings)[2],byrow = FALSE) 
  }
  ##
  p_umap<-NULL
  umap_embedding<-NULL
  if(run_umap){
    suppressWarnings(VMPLN_intseurat<- RunUMAP(VMPLN_intseurat, reduction = "pca", dims = 1:min(npc,dim(VMPLN_intseurat@reductions$pca@cell.embeddings)[2]),verbose = verbose))
    
    if(!is.null(label_umap)){
      VMPLN_intseurat$seurat_clusters<-label_umap
      p_umap <- DimPlot(VMPLN_intseurat, reduction = "umap", group.by = "seurat_clusters", label = TRUE,
                        repel = TRUE) + NoLegend()
    }
    umap_embedding<-matrix(as.vector(VMPLN_intseurat@reductions$umap@cell.embeddings),nrow = dim(VMPLN_intseurat@reductions$umap@cell.embeddings)[1],ncol = dim(VMPLN_intseurat@reductions$umap@cell.embeddings)[2],byrow = FALSE)
  }
  ...
  if(is.null(celltypes_ref)){
    if(cluster_method == "SNN"){
      ...
      VMPLN_intseurat<-FindNeighbors(VMPLN_intseurat, dims = 1:npc,verbose = verbose)
      VMPLN_intseurat<-FindClusters(VMPLN_intseurat, resolution = resolution,verbose = verbose)
      resolution_0<-resolution
      if(!is.null(celltypes_num)){
        while (length(unique(VMPLN_intseurat$seurat_clusters)) != celltypes_num) {
          if(length(unique(VMPLN_intseurat$seurat_clusters)) > celltypes_num){
            resolution_0<-resolution_0 * 0.9
            VMPLN_intseurat<-FindClusters(VMPLN_intseurat, resolution = resolution_0,verbose = verbose)
          }else{
      ...
      celltypes_label<-as.numeric(VMPLN_intseurat$seurat_clusters) 
    ...
    if(cluster_method == "Kmeans"){
      ...
      VMPLN_intseurat$seurat_clusters<-celltypes_label
      pi_vec<-as.vector(table(celltypes_label))/length(celltypes_label)
    }
  }else{
  ...
  for(j in 1:length(gene_model)){
    gene_model_index<-c(gene_model_index,which(rownames(VMPLN_intseurat@assays$RNA@counts) == gene_model[j]))
  }
  ##-----------------------------------------------------------------------------
  ...
  ##-----------------------------------------------------------------------------
  expression_count_choose<-as(Matrix::t(VMPLN_intseurat@assays$RNA@counts[gene_model_index,]),"sparseMatrix")
  rownames(expression_count_choose)<-colnames(VMPLN_intseurat@assays$RNA@counts)
  rm(VMPLN_intseurat);gc()
  ...
  VMPLN_list = list(celltypes_label = celltypes_label - 1,
                    U_mat = U_mat,
                    mu_mat = mu_mat,
  ...
  ##-----------------------------------------------------------------------------
  VMPLN_list<-init_Theta(VMPLN_list,lambda_use = 1e-6, penalize_diagonal = FALSE, Theta_threshold = 1e-4,core_num = min(celltypes_num,core_num))
  ##-----------------------------------------------------------------------------
